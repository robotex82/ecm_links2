...
describe 'ActiveRecord::Base models' do
  ActiveRecord::Base.descendants.map(&:to_s).reject { |m| %w(ActiveAdmin::Comment AdminUser Ecm::Downloads::Download Ecm::Download::DownloadCategory).include?(m) }.each do |model_name|
